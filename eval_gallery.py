...
def convert(label,char_radical_dict,alp2num):
    r_label = []
    s_label = []
    batch = len(label)
    for i in range(batch):
...
def val(model,test_loader,char_radical_dict,alp2num,char_3755,gallery_lmdb_path,imageH=128,imageW=128):
    print("Start Eval!")
    ...
    tmp_text = convert(char_3755,char_radical_dict,alp2num)
    text_features = []
    iters = len(tmp_text) // 100
    with torch.no_grad():
        ...
        with trange(test_loader_len) as t:
            for iteration in t:
                ...
                _, standard_char_features, _ = model(image, standard_char_features, test=True)
                logits_per_image_t2i = logit_scale.tolist() * image_features @ text_features.t()  # logit_scale.tolist() logit_scale.item() 
                probs_t2i, index_t2i = logits_per_image_t2i.softmax(dim=-1).max(dim=-1)

                logits_per_image_i2i = logit_scale.tolist() * image_features @ standard_char_features.t()
                probs_i2i, index_i2i = logits_per_image_i2i.softmax(dim=-1).max(dim=-1)
                for i in range(len(label)):
                    if probs_i2i[i]>probs_t2i[i]:
                        index_combine = index_i2i[i]
                        combine_char_dict=standard_char_gallery
                    else:
                        index_combine=index_t2i[i]
                        combine_char_dict=char_3755
                    if traditional_to_simplify(char_3755[index_t2i[i]]) == traditional_to_simplify(label[i]):
                        correct_t2i += 1

                    if traditional_to_simplify(standard_char_gallery[index_i2i[i]]) == traditional_to_simplify(label[i]):
                        correct_i2i += 1
                    if traditional_to_simplify(combine_char_dict[index_combine]) == traditional_to_simplify(label[i]):
                        correct_combine += 1
                    total += 1
                t.set_description('{}/{}'.format(iteration, test_loader_len))
                t.set_postfix(acc=(max([correct_i2i,correct_t2i,correct_combine]) / total))
                # t.set_postfix(acc_t2i=correct_t2i/total)
                # t.set_postfix(acc_combine=correct_combine/total)
        print(total)
        print("ACC : {}".format(max([correct_i2i,correct_t2i,correct_combine]) / total))
        print("ACC_i2i : {}".format(correct_i2i / total))
        print("ACC_t2i : {}".format(correct_t2i / total))
...
if __name__=='__main__':
    ...
    radical_alphabet_path='data/radical_27533.txt'
    ...
    ckpt_path='history/【09-26】vit_以标准字库为中介_权重均为1_clipgrad_loss_combine_bytedata_加速_yhy/best_model.pth'
    vision_type='vit'
    ...
    val(model,test_loader,char_radical_dict,alp2num,char_alpha,gallery_lmdb_path)
