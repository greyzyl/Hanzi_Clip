...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor,attn_mask):
        N,L,_=attn_mask.shape
        attn_mask = attn_mask.unsqueeze(1).expand(-1, self.n_head, -1, -1)
        attn_mask = attn_mask.contiguous().view(N * self.n_head, L, L) 
...
class FusionBlock(nn.Module):
    def __init__(self,context_length,context_length_s, attn_mask=None):
        ...
        self.trans = ResidualAttentionBlock(512, 4)

    def forward(self, x,text_2_stroke_cross_attn_mask,stroke_2_text_cross_attn_mask, fused_tensor_mask):
        cls = x[:, 0, ].unsqueeze(1)
        # text_split = x[:, 1:, ].chunk(2, dim=1)##有问题
        # text = text_split[0]
        # text_s = text_split[1]
        text=x[:,1:1+self.context_length]
        text_s=x[:,1+self.context_length:1+self.context_length+self.context_length_s]
        # print('in fusionblock')
        # print(text_2_stroke_cross_attn_mask,stroke_2_text_cross_attn_mask, fused_tensor_mask)
        # padding = (1, 0, 0, 1) 
        # text_2_stroke_cross_attn_mask=F.pad(text_2_stroke_cross_attn_mask, padding, mode='constant', value=0)
        word_image_align, _ = self.multihead(text, text_s, text_s, mask=text_2_stroke_cross_attn_mask)
        result = self.mul_layernorm2(text + word_image_align)
        result = self.mul_layernorm3(result + self.pff(result)) # b l c
        # stroke_2_text_cross_attn_mask=F.pad(stroke_2_text_cross_attn_mask, padding, mode='constant', value=0)
        word_image_align_1, _ = self.multihead_1(text_s, text, text, mask=stroke_2_text_cross_attn_mask)
        ...
        # print(word_image_align)
        # print(word_image_align_1)
        # exit()
        res = torch.cat([cls, result, result_1], dim=1) # b 2l c
        res = res.permute(1, 0, 2)

        # print(fused_tensor_mask)
        padding = (1, 0, 0, 0) 
        fused_tensor_mask=F.pad(fused_tensor_mask, padding, mode='constant', value=0)##有问题
        first_row = fused_tensor_mask[:, 0:1, :] 
        fused_tensor_mask = torch.cat([first_row, fused_tensor_mask], dim=1)
        res = self.trans(res,attn_mask=fused_tensor_mask)
        res = res.permute(1, 0, 2)

        return res

class FusionLayer(nn.Module):
    def __init__(self, layers,context_length,context_length_s):
        super().__init__()
        self.layers = layers
        self.context_length=context_length
        self.context_length_s=context_length_s
        self.resblocks = nn.Sequential(*[FusionBlock(context_length,context_length_s) for _ in range(layers)])

    def forward(self, x: torch.Tensor, text_2_stroke_cross_attn_mask,stroke_2_text_cross_attn_mask, fused_tensor_mask):
        for resblock in self.resblocks:
            x=resblock(x,text_2_stroke_cross_attn_mask,stroke_2_text_cross_attn_mask, fused_tensor_mask)
            # print('in fusionlayer')
            # print(x)
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        self.positional_embedding_s = nn.Parameter(torch.empty(self.context_length_s, transformer_width))
        ...
        self.positional_embedding_fl = nn.Parameter(scale * torch.randn( self.context_length_s+self.context_length + 1, transformer_width))
        self.ln_pre = LayerNorm(transformer_width)
        # self.fl = FusionLayer(3, attn_mask=self.build_attention_mask_fuse())
        self.fl = FusionLayer(3,context_length,context_length_s )
    ...
    def build_attention_mask(self):
        ...
        return mask
    def build_attention_mask_s(self):
        # lazily create causal attention mask, with full attention between the vision tokens
        # pytorch uses additive attention mask; fill with -inf
        mask = torch.empty(self.context_length_s, self.context_length_s)
        mask.fill_(float("-inf"))
        mask.triu_(1)  # zero out the lower diagonal
        return mask
    def build_attention_mask_fuse(self):
        # lazily create causal attention mask, with full attention between the vision tokens
        # pytorch uses additive attention mask; fill with -inf
        mask = torch.empty(self.context_length_s+self.context_length + 1, self.context_length_s+ self.context_length + 1)
        mask.fill_(float("-inf"))
        mask.triu_(1)  # zero out the lower diagonal
        return mask
    ...
    def encode_text(self, text,attn_mask):
        x = self.token_embedding(text).type(self.dtype)  # [batch_size, n_ctx, d_model]

        x = x + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)  # NLD -> LND
        # print('in encode_text')
        # print(attn_mask)
    ...
    def encode_all(self, text_features, text_features_s,text_2_stroke_cross_attn_mask,stroke_2_text_cross_attn_mask, fused_tensor_mask):
        ...
        all_features_cls = self.fl(all_features_cls,text_2_stroke_cross_attn_mask,stroke_2_text_cross_attn_mask, fused_tensor_mask)
    ...
    def forward(self, image, text, text_s,text_tensor_mask, stroke_tensor_mask, fused_tensor_mask,text_2_stroke_cross_attn_mask,stroke_2_text_cross_attn_mask, text_features_fuse=None, test=False):
        image_features = self.encode_image(image)
        if test == False:
            text_features, seq_text = self.encode_text(text,text_tensor_mask)
            text_features_s, seq_text_s = self.encode_text_s(text_s,stroke_tensor_mask)
            text_features_fuse = self.encode_all(seq_text, seq_text_s,text_2_stroke_cross_attn_mask,stroke_2_text_cross_attn_mask, fused_tensor_mask)
        else:
            text_features = text
            text_features_s = text_s
            text_features_fuse = text_features_fuse
        # print('in clip')
        # print(text_features_fuse)
        ...
        return image_features, text_features, text_features_s, text_features_fuse, logit_scale, logit_scale_s, logit_scale_all
